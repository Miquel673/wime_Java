...
include __DIR__ . "/conexion.php"; // Corregido
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO usuario (NombreUsuario, EmailUsuario, ContrasenaUsuario, Edad, FechaRegistro) 
                            VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssis", $nombre, $email, $hash, $edad, $fecha);

    if ($stmt->execute()) {
        echo "<script>alert('✅ Registro exitoso.'); window.location.href='/Wime/public/HTML/Wime_interfaz_inicio-Sesion.html';</script>";
    } else {
